									...
									<tbody>
										...
										foreach($arBasketIDs as $arItem){
											if(($arItem["CAN_BUY"] == "Y") && ($arItem["DELAY"] == "N")){
												...
												$item=CNextCache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CNextCache::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array("ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arItem["PRODUCT_ID"]), false, false, array("ID", "DETAIL_PAGE_URL", "IBLOCK_ID", "PROPERTY_CML2_LINK", "PREVIEW_PICTURE", "DETAIL_PICTURE"));
												if($item["PROPERTY_CML2_LINK_VALUE"]){
													$itemLink=CNextCache::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CNextCache::GetIBlockCacheTag($item["IBLOCK_ID"]))), array("ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $item["PROPERTY_CML2_LINK_VALUE"]), false, false, array("ID", "PREVIEW_PICTURE", "IBLOCK_ID", "DETAIL_PICTURE"));
													if($itemLink){
														$item["ID"] = $itemLink["ID"];
